...
function KingWithMovingEyes({ position }: Props) {
  ...
  const rotateEyes = (mouseX: number, mouseY: number) => {
    if (leftEye.current != null && rightEye.current != null) {
      ...
      let angleLeftEye: number =
        (Math.atan2(rect1.y - mouseY, rect1.x - mouseX) * 180) / Math.PI;
      angleLeftEye += 180;
      leftEye.current.style.transform = `rotate(${angleLeftEye}deg)`;

      const rect2 = rightEye.current.getBoundingClientRect();
      let angleRightEye: number =
        (Math.atan2(rect2.y - mouseY, rect2.x - mouseX) * 180) / Math.PI;
      angleRightEye += 180;
  ...
  return (
    <div>
      <img alt="king" src="cards/king.png" className={position}></img>
      <img
        alt="left_eye"
        className="left_eye"
        src="cards/eye.png"
